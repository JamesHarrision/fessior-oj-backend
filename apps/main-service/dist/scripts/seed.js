...
async function main() {
    ...
    await testcase_model_1.Testcase.deleteMany({});
    console.log('Clean up done.');
    // 1. Seed Users
    console.log('Seeding users...');
    const defaultPassword = await (0, password_util_1.hashPassword)('password123');
    ...
    const seedProblem = async (problemData, testcasesData, tagsList) => {
        console.log(`Creating problem: ${problemData.title} in MongoDB...`);
        const p = new problem_model_1.Problem(problemData);
        await p.save();
        ...
        await testcase_model_1.Testcase.insertMany(tcs);
        return probIdx;
    };
    // Two Sum (Math)
