...
export const executeInDocker = async (containerName, code, language, stdin, expectedOutput, timeLimitMs) => {
    ...
    const execInContainer = (command, inputData, timeoutMs) => {
...
export const executeOnHost = async (code, language, stdin, expectedOutput, timeLimitMs) => {
    ...
    return new Promise((resolve) => {
        ...
        const startTime = Date.now();
        const child = exec(command, { timeout: timeLimitMs }, (error, stdout, stderr) => {
            const elapsed = Date.now() - startTime;
            try {
                fs.unlinkSync(filePath);
            }
            ...
            if (language === 'cpp') {
                try {
                    fs.unlinkSync(filePath.replace('.cpp', '.exe'));
                }
                catch { }
            }
            if (error) {
...
export const executeLocally = async (code, language, stdin, expectedOutput, timeLimitMs) => {
    const containerName = (() => {
        try {
...
export const executeTestCase = async (code, languageId, stdin, expectedOutput, timeLimitMs, config) => {
    const { judge0Url, rapidApiKey, rapidApiHost, enableLocalFallback = false } = config;
    ...
    try {
        ...
        const isSandboxError = statusId === 13 ||
            (statusId === 6 && (compileOutput.includes('Failed to create control group') ||
        ...
        if (isSandboxError) {
            const judge0Error = compileOutput || 'Judge0 sandbox/runtime error. Check judge0-workers logs for isolate/cgroup failures.';
            ...
            console.warn(`Judge0 returned sandbox/compilation error (statusId: ${statusId}). Local fallback is disabled.`);
            return {
                status: statusId === 6 ? 'CE' : 'RE',
                time: 0,
                memory: 0,
                error: `Judge0 execution failed (statusId: ${statusId}).\n${judge0Error}`,
                actualOutput: '',
            };
        }
        let status = 'WA';
        ...
        return {
            ...
            time: 0,
            memory: 0,
            error: `Cannot reach Judge0 API at ${judge0Url}: ${err.message}`,
            actualOutput: '',
        };
    }
};
